...
export default function BusDetailsScreen({ navigation, route }) {
    const { bus } = route.params; // Passed from ManageBusesScreen
    const { token } = useAuth();
    const [loading, setLoading] = useState(false);

    // Initialize state with passed bus data
    const [busCode, setBusCode] = useState(bus.busCode);
    const [numberPlate, setNumberPlate] = useState(bus.numberPlate);
    const [district, setDistrict] = useState(bus.district);
    ...
    const handleUpdate = async () => {
        ...
        try {
            setLoading(true);
            await axios.put(
                `${API_URL}/admin/edit-bus/${bus._id}`,
                {
                    busCode,
                    numberPlate,
                    district,
            ...
            Alert.alert("Error", err.response?.data?.error || "Failed to update bus");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        ...
        >
            <ScrollView contentContainerStyle={styles.scrollContent}>
                <Text style={styles.headerTitle}>Edit Bus: {bus.busCode}</Text>
